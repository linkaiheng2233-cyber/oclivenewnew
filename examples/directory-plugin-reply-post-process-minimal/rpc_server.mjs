/**
 * Minimal directory plugin: reply_post_process.process — prefixes display reply.
 * stdout: OCLIVE_READY http://127.0.0.1:<port>/rpc
 */
...
function handleReplyPostProcess(params) {
  const raw = params && typeof params.raw_reply === "string" ? params.raw_reply : "";
  return {
    display_reply: `[dir-pp] ${raw}`,
    diagnostic: "directory-plugin-reply-post-process-minimal",
  };
}
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (msg.method === "reply_post_process.process") {
      res.writeHead(200);
      res.end(jsonRpcResult(id, handleReplyPostProcess(msg.params)));
      return;
    }
    res.writeHead(200);

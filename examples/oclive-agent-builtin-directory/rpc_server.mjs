/**
 * 演示 `agent.process`（非完整 ReAct）；验证 directory 槽 JSON-RPC 契约。
 */
...
function handleAgentProcess(params) {
  const msg = params && params.message != null ? String(params.message).trim() : "";
  if (!msg) {
    return { handled: false, reply: "" };
  }
  return {
    handled: true,
    reply: `[com.oclive.builtin.agent demo] echo: ${msg.slice(0, 200)}`,
  };
}
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (msg.method === "agent.process") {
      res.writeHead(200);
      res.end(jsonRpcResult(id, handleAgentProcess(msg.params)));
      return;
    }
    res.writeHead(200);

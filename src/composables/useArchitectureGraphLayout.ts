...
export function defaultSizeForNode(id: string, type?: string) {
  if (type && ARCH_NODE_DEFAULT_SIZE[type])
    return ARCH_NODE_DEFAULT_SIZE[type]
  if (id.startsWith('plugin:'))
    return ARCH_NODE_DEFAULT_SIZE.archPlugin!
...
export function useArchitectureGraphLayout() {
  ...
  function load(): void {
    try {
      ...
      if (raw)
        layout.value = JSON.parse(raw) as Record<string, NodeLayoutEntry>
    }
    catch {
      layout.value = {}
    }

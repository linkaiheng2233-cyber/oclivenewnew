...
import type { SlotGroupsMap, SlotRegistryEntry, SlotRegistryMap } from '../lib/slotRegistry'
...
import { normalizeBackendKind } from '../lib/graphEditorTheme'
...
export function useArchitectureGraphModel() {
  ...
  function visiblePluginIds(key: string, entry?: SlotRegistryEntry): string[] {
    if (usesBlueprint.value && entry) {
      ...
      if (all.length === 0)
        return []
      if (expandedPlugins.value[key] || all.length <= 1)
        return all
      return [all[0]!]
    }
    const all = directoryPluginIds(key as CoreModule)
    if (backendKind(key as CoreModule) !== 'directory' || all.length === 0)
      return []
    if (expandedPlugins.value[key] || all.length <= 1)
      return all
    return [all[0]!]
  ...
  function buildBlueprintNodes(registry: SlotRegistryMap): Node[] {
    ...
    interface AbsPos { absX: number, absY: number, ring: ReturnType<typeof layoutOnRing> }
    ...
    interface GroupBounds { minX: number, minY: number, maxX: number, maxY: number, type: string, label: string }

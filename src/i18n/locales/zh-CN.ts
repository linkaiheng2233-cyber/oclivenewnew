export const zhCN = {
  ...
  settings: {
    ...
    cloudLlm: {
      title: "云端对话模型（OpenAI 兼容）",
      hint:
        "在此填写 Base URL、API Key、模型 ID（如 DeepSeek：`https://api.deepseek.com` + `deepseek-chat`）。Base URL 不要带 `/v1`。环境变量 `OCLIVE_CLOUD_LLM_*` 优先。关闭「允许云端 API」可硬性禁止 OpenAI 兼容出站（本地 Ollama 仍可用）。需在插件权限中为 `system:remote_llm_http` 授予 `network:*`；首次保存前请阅读安全说明并勾选确认。",
      baseUrl: "Base URL",
      apiKey: "API Key",
      apiKeyPlaceholder: "留空表示保留已保存的 Key",
      apiKeySavedHint: "已保存过 Key；留空保存将保留原 Key。",
      model: "模型 ID",
      ...
      verify: "测试连接",
      clear: "清除已保存",
      clearConfirm: "确定清除本机已保存的云端 Base URL / API Key / 模型等凭据？（策略开关可保留）",
      savedToast: "云端 LLM 设置已保存。",
      clearedToast: "已清除应用内保存的云端凭据。",
      verifyOkToast: "连接成功。",
      autoRemoteLabel: "云端就绪时自动使用远程 LLM（包内为 Ollama 时提升为 remote）",
      ackLabel: "我已了解：请求将发往公网，API Key 存于本机数据库；我可随时在下方关闭云端或清除凭据。",
      securityBanner: "你尚未勾选安全确认；保存凭据前请先勾选。",
      netWarn: "尚未授予 `system:remote_llm_http` 的 `network:*`，自动提升不会生效；请到插件与后端管理授权。",
    },
    shortcuts: {
      label: "快捷",
  ...
  builtinLlamaModels: {
    ...
    cloudGate: {
      title: "云端 API 总闸",
      hint: "关闭后一律不发起 OpenAI 兼容云端请求。详细配置在「设置 → 常规 → 云端对话模型」。",
      allow: "允许云端 API（OpenAI 兼容）",
      toastSaved: "已更新云端总闸。",
    },
    guide: {

...
export function isSystemEdge(edge: Pick<Edge, 'id' | 'data' | 'deletable'>): boolean {
  if (edge.deletable === false)
    return true
  if (SYSTEM_EDGE_IDS.has(String(edge.id)))
    return true
...
export function validateArchConnection(
  ...
  if (source === target)
    return { valid: false, reason: 'selfLoop' }

  const nodesById = new Map(nodes.map(n => [n.id, n]))

  if (source === ARCH_GRAPH_KERNEL_ID && target === ARCH_GRAPH_BUS_ID) {
    if (sourceHandle === 'pipeline' && targetHandle === 'pipeline-in')
      return { valid: true }
    return { valid: false, reason: 'unknownPort' }
  }
  ...
  if (source === ARCH_GRAPH_BUS_ID && CORE_MODULE_IDS.has(target)) {
    ...
    if (sourceHandle === `fac-${mod}` && targetHandle === 'backend-in')
      return { valid: true }
    return { valid: false, reason: 'unknownPort' }
  }

  if (source === ARCH_GRAPH_BUS_ID && target === ARCH_GRAPH_COMPLEX_ID) {
    if (sourceHandle === 'fac-complex' && targetHandle === 'backend-in')
      return { valid: true }
    return { valid: false, reason: 'unknownPort' }
  }
  ...
  if (CORE_MODULE_IDS.has(source) && target.startsWith('plugin:')) {
    ...
    if (pluginMod !== mod)
      return { valid: false, reason: 'pluginModuleMismatch' }
    return { valid: true }
  }
...
function inferEdgeId(connection: Connection): string {
  if (connection.source === ARCH_GRAPH_KERNEL_ID)
    return 'kernel-bus'

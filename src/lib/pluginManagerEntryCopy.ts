...
export function settingsGeneralLeadHtml(): string {
  return (
    ...
    "<strong>Ctrl+Shift+A</strong> 打开<strong>插件市场</strong>（V1/V2 一致）。"
  );
}
...
export function settingsExperimentalToggleDescriptionHtml(): string {
  return (
    "开启后，<strong>Ctrl+Shift+F</strong> 与顶栏「更多」里的 <strong>Oclive Manager</strong> 入口将<strong>打开并切换 V2 预览</strong>（设置会记住此项）。" +
...
export function settingsShortcutsHelpHint(): string {
  return "Ctrl+Shift+S 打开设置；Ctrl+Shift+F 打开 Oclive Manager（V1/V2 由下方实验性勾选决定）；Ctrl+Shift+A 打开插件市场；Ctrl+Shift+D 开关调试面板。";
}
...
export function moreMenuTileHelpText(experimentalV2: boolean): string {
  if (experimentalV2) {
    return (
      ...
      "Ctrl+Shift+F 与下方按钮打开 Oclive Manager（V2 预览）；Ctrl+Shift+A 打开插件市场；在设置中关闭「V2 预览」可恢复专业模式（V1）。" +
      TAIL_DEBUG_SHORTCUT
    );
  }
  return (
    "将快捷键说明、设置页与 Oclive Manager（插件与后端管理）集中到同一处。快捷键：Ctrl+Shift+S 打开设置；" +
    "Ctrl+Shift+F 打开专业模式（V1）Oclive Manager（含开发者调试）；Ctrl+Shift+A 打开插件市场。" +
    TAIL_DEBUG_SHORTCUT
  );
}
...
export function shortcutHelpCtrlShiftADescription(): string {
  return "打开插件市场（V1/V2 一致）：在线索引、安装、模块/Profile、本地投放导入等";
}

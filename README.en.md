...
| **CI gates** | **`rustfmt` + `clippy` (`-D warnings`) + `cargo test`** (`src-tauri`) + **`npm ci` / `npm run test:unit` / `npm run build`**; **`oocp-test-suite`** (OOCP S0–S12 + optional S13 + core HTTP restart smoke), **`cargo-audit`** (allow-fail), **remote-plugin-demo**. |

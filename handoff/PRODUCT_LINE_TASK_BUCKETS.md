...
> **状态（批次一）**：**A5.1** 兼容一页表、**A1.3** CI/`check:release` 说明强化、**A2.1** 文档子集（ERROR_CODES §1.5）、**A4.1** 高风险演示表（`PLUGIN_HIGH_RISK_ACCEPTANCE`）、**A8.1** 快捷键文档（README「测试与检查」）已落。**A6.1 / A3.2 / A8.2** 仍以代码/交互切片跟进，见 §四与主清单 §A。
...
| **A3.2（切片）** | **`[CODE]` + 前端映射**：列 5～10 个高频 `invoke` 错误，Rust 不直出整句中文 | 可持续开 issue 列表，逐条勾 |
...
| **A3.1** | **崩溃与遥测**：Sentry 或等价，默认关、隐私说明、脱敏、可关闭，与 README 一致 | 产品与工程共同决策 |
...
|----------|------------|--------|----------------|
| 1 | **A1.1** | **核心路径自动化**（装→启→切角→发消息→重启）稳定、防 flake、进 CI | **PoC 已入库**：[`scripts/e2e-core-api-restart.mjs`](../../scripts/e2e-core-api-restart.mjs) + CI `oocp-test-suite` 串联；全桌面 / GUI 仍后续扩展 |
| 2 | **A1.2** | **`invoke` 全矩阵**或契约对照数据集，覆盖面大 | 在 A1.1 PoC 之后：先列 **Top 10 invoke** 与 golden 响应 |
...
4. **并行立项**：**A1.1** 已落 **HTTP 进程重启 PoC**（`scripts/e2e-core-api-restart.mjs` + CI）；下一步：GUI / 安装器路径或 Playwright（另开 issue）。

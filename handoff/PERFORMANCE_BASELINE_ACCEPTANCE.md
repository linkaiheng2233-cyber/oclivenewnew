...
**前端体积对照（Vite 生产构建，约 2026-05）**：主入口 `index-*.js` 约 **365 kB / gzip ~118 kB**；插件/市场/设置/调试及场景相关壳多为 **async chunk + `v-if`**，首屏不解析。进一步分析：`npm run build:analyze`，打开 **`dist/stats.html`**（`dist/` 已 `.gitignore`，勿提交）。

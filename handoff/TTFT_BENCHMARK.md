...
固定轮次与真实时长模式都并发运行 LLM/TTS；真实时长从 CosyVoice 热身完成后计时，保证至少完成一组。schema v3 除 `requested_duration_minutes` / `actual_load_seconds` / `pairs_completed`、GPU 采样与清理证据外，还报告 Sidecar 首块、客户端交付开销、prompt cache，以及 `pre_token2wav_wait` / `first_token2wav` 等分段分布。`--voice-text` 可做首个短分句与较长片段矩阵；`--voice-first-chunk-priority on|off`、`--voice-first-hop-tokens`、`--llm-priority` 与 `--llm-poll` 只用于复现实机调度 A/B，报告会记录模型级配置及每次请求是否实际命中有限优先策略。warm 与实测使用同一 emotion/ref 文本，证据只记录字符数。`--output` 原子替换最终报告，并按 `--checkpoint-interval-seconds` 刷新同目录 `*.checkpoint.json`；checkpoint 保持有界，只记录最近一次请求、进度、GPU 采样、子进程 PID/存活和最终 cleanup。Ctrl+C 会写 `interrupted` 与退出码 130；断电/强杀仍只能保留最后一次已落盘 checkpoint。任一进程提前退出、未回收或性能/余量门禁失败均非零退出。
...
2026-08-02 K-VOICE-09 首块优先短矩阵（实现 `a1211f97`、证据工具 `4a94b063`，同机 **8151MiB**）：维护者确认采用“首声优先，可接受少量后续吞吐成本”。仅提升 CosyVoice CUDA stream 优先级在长/短各 10 组中没有稳定改善总体 TTFC；降低 llama GPU 层数、进程优先级与 poll 同样未形成可靠收益，说明跨进程竞争不能靠单一优先级参数解决。最终有限策略只对不少于 **10** 字符的首段启用独立高优先级 speech-token stream、首个 waveform stream 与 **20-token** 初始 hop，后续仍按上游规则扩至 40/80；更短首段保留上游 **25-token** 单块路径。长段 10 组基线 TTFC p50/p95(max) **4135/4473ms**，有限策略复测为 **3643/3826ms**，分别下降 **492ms（11.9%）/647ms（14.5%）**；总耗时 p50 **6085→5564ms**，10/10 请求命中 20-token 档、流块均为 3。峰值 **6757/8151MiB**、余量 **1394MiB**、稳态增长 **0MiB**、进程均回收。4 字短句 bounded 复测 10/10 未启用策略、hop 均为 25、块数均为 1；8-token 试验会把部分短句拆成 2 块并把总耗时 p95 拉到 **3522ms**，已否决。以上仍是短矩阵，不替代既定 **30 分钟**真实矩阵与用户听感验收；8 秒绝对门禁未放宽。

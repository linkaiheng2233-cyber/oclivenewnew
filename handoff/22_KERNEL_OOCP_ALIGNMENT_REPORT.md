...
| 编号 | 事项 | 状态 | 核心产出 |
|------|------|------|----------|
| P0-A | 内核边界文档 + 代码分层骨架 | ✅ 完成 | `creator-docs/kernel/KERNEL_BOUNDARY.md`、`KERNEL_ENTRY_CHECKLIST.md`、`src-tauri/src/domain/core/` 分层 |
| P0-B | OOCP v0.1 协议规范 | ✅ 完成 | `creator-docs/oocp/OOCP_SPEC_v0_1.md`（消息模型 / 错误码 / 方法白名单 / 鉴权） |
| P0-C | OOCP 最小实现（传输无关 handler + Tauri adapter） | ✅ 完成 | `src-tauri/src/domain/core/oocp_handler.rs`（`dispatch_oocp_request` + `OocpMethodHandler` trait）+ `src-tauri/src/domain/adapters/tauri_oocp_handler.rs` |
...
| `src-tauri/src/domain/mod.rs` | domain 模块重组（`core/` + `adapters/`） |
| `src-tauri/src/domain/core/mod.rs` | 内核核心模块（传输无关 handler 所在层） |
| `src-tauri/src/domain/core/oocp_handler.rs` | OOCP 传输无关分发器 + `OocpMethodHandler` trait |
...
| `src-tauri/src/models/oocp.rs` | OOCP 消息类型（`OocpRequest`/`Response`/`Event`/`Error`/`Capabilities` + 错误码枚举） |
...
| `src-tauri/src/models/oocp.rs` | capabilities 常量改为 `pub use oclive_core::capabilities::{...}` 重新导出 |
...
┌──────────────▼───────────────────────┐
│  src-tauri/src/domain/core/          │  ← 内核核心（传输无关）
...
- `src-tauri/src/models/oocp.rs` 保留 OOCP 消息结构体（含 serde 标注），同时从 `oclive_core` 重新导出 capabilities 常量。
...
2. **`oclive_core` 类型统一**：将 `src-tauri/src/models/oocp.rs` 中的 OOCP 结构体也改为从 `oclive_core` 重新导出（当前仅 capabilities 常量已迁移）。

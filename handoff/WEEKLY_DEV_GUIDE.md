...
- **首包 / chunk 基线（v0.2）**：✅ 已落地（`App.vue` 懒加载壳、`vite` `manualChunks`、`main.js` Sentry 延后、根 `Cargo.toml` `profile.release`）；明细见 **`FRONTEND_CHUNK_OPTIMIZATION.md`**、**`RUST_RELEASE_AND_DEPENDENCIES.md`**。`reqwest` 去 `blocking` 等属**可选**后续，勿阻塞功能交付。

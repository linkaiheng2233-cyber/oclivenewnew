...
- **开发期先窄测**：每个切片跑受影响测试并做本地提交；不把远端全量 CI 当作编译器、单测替代品。
- **按里程碑推送**：普通开发先在本地累积逻辑完整、可审查的提交；只有协作交接、备份或里程碑需要时才推送。未冻结分支需要远端同步时，优先不创建 ready PR，避免每次同步触发全矩阵。
- **冻结 HEAD 再全量**：L 级、发版、main 恢复或技术债转 Done，先完成 applicable 本地门禁，再冻结目标 HEAD、一次推送并等待该 SHA 的远端 CI。失败后只做针对失败根因的修复与窄测，修复完成再推送。

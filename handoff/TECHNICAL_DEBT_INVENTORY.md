...
| **K-RESOURCE-COORD-01** | llama-server / Ollama / CosyVoice 与未来 Live2D 各自决策资源，缺少统一预算和租约 | **P1** | 内部 `ExecutionPlan` + Resource Coordinator；LLM/Voice 两适配器先闭环 snapshot/admission/lease/pressure/degrade/release；覆盖不可控外部进程、取消、故障恢复与真实共享显存压力测，再接第三类渲染适配器 | **Partial · Stage 2.2 locally verified**（2026-07-30：Stage 2.1 确认式 CosyVoice `unload` 与适配器串行锁之上，Performance LLM 已加入受协调 fallback admission gate：资源暂停拒绝新 Ollama fallback、等待在途请求排空、卸载 OCLive 追踪模型；普通/标签/opts/流式/启动探针同门，provider 暂停仍保留普通 fallback，显式预热/模型应用负责恢复；并发恢复只登记为 supersede，排空/卸载完成前不会提前开门。定点 `performance_llm` **10** 测与 host lib Clippy 通过。当前仍为 `coordinator_selectable=false`；Voice 尚未发起自动暂停、失败回滚和释放后恢复。仍缺完整自动抢占/恢复与多档切换、第三方入口、不可控外部进程故障矩阵、真实共享显存 stress/soak、RAM/CPU 与第三类渲染适配器；未升 Done） |

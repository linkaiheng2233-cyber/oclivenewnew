...
| **V-MODULE-QUALITY-01** | “上限 = 模块上限”缺少组件质量对比 harness | **P1** | 固定角色/场景/replay 输入；可复现比较 memory / emotion / prompt / LLM 模块的质量与行为指标，而非只比延迟 | **OPEN · Stage 2 Locally verified**（[Full 计划](./debt-marathon/long-plans/V-MODULE-QUALITY-01.md)；Stage 1 `675561d5` 冻结版本化 fixture/评分合同；Stage 2 `c883ceae` 复用真实 HTTP/replay/remote-slot 链采集，3/3 cases、memory 7/7、emotion 3/3、prompt 9/9、LLM 11/11，非 fixture 记忆与完整生产 Prompt 不出临时进程；仍待 Stage 3 至少两套模块配置对比、双语文档与 Stage 4 精确提交远程 CI，不升 Done） |

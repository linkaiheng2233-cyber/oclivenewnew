...
| **K-SUPPLY-12** | npm 开发工具链仍有 6 项 audit 命中，且 ESLint 9.39.5 不满足 `eslint-plugin-unicorn` 68 的 ESLint ≥10.4 peer 契约 | **P1** | 对 ESLint/`brace-expansion`、WebDriver/`fast-xml-parser`、dev-only `vue3-sfc-loader` 旧 Vue/PostCSS 链逐条做可达性与升级路径；对齐受支持版本后 `npm ls eslint eslint-plugin-unicorn` 退出 0，完整 `npm audit` 无 high（或有逐项时限豁免），lint/typecheck/unit/build 与 Linux/Windows CI 全绿；禁止 `--force` 或无证据 override | **OPEN · remote measured**（2026-08-01：生产门禁为 **0 vulnerabilities**，但同一锁文件完整扫描为 **3 moderate / 3 high**；Node 22 已解决运行时要求，peer 与 dev-tool 供应链尚未解决） |
...
**下一工程动作**：先处理 **K-SUPPLY-12** 的受支持版本矩阵与 dev-tool 可达链；插件发布信任仍以 **K-SUPPLY-09** 为主要 P1。

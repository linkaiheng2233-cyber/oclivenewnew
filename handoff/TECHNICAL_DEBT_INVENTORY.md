...
| **V-MODULE-QUALITY-01** | “上限 = 模块上限”缺少组件质量对比 harness | **P1** | 固定角色/场景/replay 输入；可复现比较 memory / emotion / prompt / LLM 模块的质量与行为指标，而非只比延迟 | **OPEN · Stage 4 Locally verified**（[Full 计划](./debt-marathon/long-plans/V-MODULE-QUALITY-01.md)；Stage 1 `675561d5` 冻结 fixture/评分合同；Stage 2 `c883ceae` 复用真实 HTTP/replay/remote-slot 链采集，3/3 cases、四维全分且秘密隔离；Stage 3 `885b0b75` 拒绝伪配置、并列 reference 与 deterministic remote-slot 配置、行为/性能分栏、双语文档与 Dimension 5 27/27；Stage 4 已完成 Action Node 24、兼容依赖锁与全组成门禁本地验证。仍待 exact-head 远程 CI；两套配置是可复现合同基线，不冒充两套真实生产模型的主观质量结论） |

...
**Last updated:** 2026-07-27（R18 staged beat 前置协议、后台连续预生成队列与本机真实模型/共享显存性能矩阵已完成）
...
| **V-MODULE-QUALITY-01** | “上限 = 模块上限”缺少组件质量对比 harness | **P1** | 固定角色/场景/replay 输入；可复现比较 memory / emotion / prompt / LLM 模块的质量与行为指标，而非只比延迟 | **OPEN**（[Full 计划](./debt-marathon/long-plans/V-MODULE-QUALITY-01.md) Ready · Stage 0 已完成；已有 OOCP、replay、MOCK_LLM 与 CLI bench 零件，Stage 1 待建立固定用例合同与离线确定性评分器） |
...
- **D5 · 普通用户身份资格（已确认 · 2026-07-27）：** 用户完成本机成年确认后，不再要求对每个普通 `user_identity` 单独确认、授权或由创作者逐个加入白名单，所有普通用户身份均可进入已启用角色的 R18 模式。
- **D6 · 显式未成年身份的简单阻止（已确认 · 2026-07-27）：** 当当前 `user_identity` 内容明确声明其为未成年人时，保留 R18 入口；用户点击后明确阻止进入，并提示切换为成年身份。不增加复杂身份白名单、自动切换或额外授权流程。
...
- **D15 · 切换用户身份自然结束当前互动（已确认 · 2026-07-27）：** 正在进行 R18 互动时切换 `user_identity`，采用与切换场景相同的收束规则：先让角色依人设自然结束并清除当前成人互动状态，再让新身份从普通聊天开始；角色应依据人设和身份变化自然回应。若目标身份明确声明为未成年人，切换本身可以完成，但该身份之后点击或尝试进入 R18 时继续按 D6 明确阻止。
...
- **D33 · 地点与成人题材不作为本地运行时审查开关（已确认 · 2026-07-27）：** 对通过成年确认且两级开关均开启的成人虚构创作，OCLive / Chat Pro 编排层不因场景位于公园、图书馆等公开地点，也不因具体成人题材类别而额外阻断、改写或降级；地点风险、第三方反应、隐蔽程度与现实后果属于角色包创作者的叙事空间。第三方本地角色包的导入校验聚焦结构、版本、成人声明与安全解析，不对其成人剧情做官方发行标准式的语义审核。此决定不撤销 D3～D6 已确认的成年资格流程和显式未成年用户身份阻止，也不要求底层模型绕过其自身能力或策略。
...
- **实施收口（2026-07-27）：** D1～D28、D30～D35 的 v1 运行时/角色包/编写器链已实现并完成本地验证。D25～D28 的前置条件不再借用正式 turn：迁移 `038_adult_staged_beats.sql` 持久保存未展示结构化文本，stage 路径跳过聊天、短期记忆、关系、事件、人格与叙事连续性写入；前台按序 commit 后才进入历史与成人短期记忆，重复 commit 使用稳定幂等键。取消与新 generation 会删除未提交拍，生成完成和取消竞态经压力用例验证不残留 pending；Chat Pro 以所有角色/聊天共享的正整数容量暂停/恢复生成，降低上限不丢已有拍，重启后可恢复或取消。**D29 继续保留**，待真实模型、上下文长度、显存/内存与长时间稳定性矩阵完成后再替换设置页中的保守建议值 `2`；未取得远端 CI 证据前状态保持 `Partial / Locally verified`。

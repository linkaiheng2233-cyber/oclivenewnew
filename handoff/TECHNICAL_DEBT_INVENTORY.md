...
| **K-CI-IMPACT-01** | 单一全量 CI 不理解 OCLive 模块语义，重复构建且无法为新增模块解释验证范围 | **P1** | Stage 1：版本化模块描述、中央影响图与受信验证目录；Rust 确定性规划器提供 `plan/explain`、未知范围全量回退；GitHub 影子 job 只产报告、不跳现有 job。Stage 2 以全量结果度量漏选/过选；证据成熟后才允许低风险 PR 选择性执行 | **In progress · Stage 1 Shadow + Compare sample 1**（2026-08-01：Stage 1 基线远端 [`30656699601`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/30656699601) 通过；首个对照样本 [`30692428026`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/30692428026) 也完成 **21/21** job。单个 `oclive.shared` 测试改动被传播到 6 个模块，规划器选 8 个验证器 / 10 个实际 job，建议跳 10 个 job（该次实跑耗时合计 **2238s**）；两轮前置失败均落在已选中的 `frontend`，本样本无已知漏选，但仍过于单一且 Rust 关键路径仍被选中，不能据此开启选择性执行。脚手架继续不拥有工作流、验证器、Runner、Secret、门禁或第二套解析器。实施 SSOT：[`SOMEDAY_TOOLCHAIN_CI.md`](../creator-docs/roadmap/SOMEDAY_TOOLCHAIN_CI.md)） |
...
| **D-CI-EXECUTION-02** | Stage 1 影子规划器尚未减少现有全量 job，通用 Rust job 又重复 CLI 嵌套 Cargo E2E；可见性 job 与硬门禁混居主 workflow | **P1** | 先按所有权去重 workspace/CLI/audit，串行或离线化嵌套 Cargo build 并稳定缓存；再把 soft visibility job 明确迁至 nightly/dispatch 或升为硬门禁；选择性执行仍须等待 Shadow 漏选/过选证据 | **Partial · remote verified ownership split**（2026-08-01：通用 Rust 排除 `oclive-cli`，CLI 独占串行嵌套 E2E 与缓存；Dimension 5 独占 `cargo audit`，`npm-audit` 升 required，frontend 持有 lint + `vue-tsc`。远端 [`30692428026`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/30692428026) 对比基线 [`30683169339`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/30683169339)：总 job-seconds **10491→8586（−18.2%）**，Linux Rust **2068→1545s（−25.3%）**，Windows Rust **3722→2528s（−32.1%）**，CLI **322→284s（−11.8%）**。Stage 1 仍只 Shadow；loom/fuzz/e2e-tauri/cli-bench/visual 等 soft job 的 nightly/dispatch 分流尚未完成，因此不升 Done） |
...
**下一工程动作**：冻结并远端验收 **K-SUPPLY-12**，随后按 **D-CI-EXECUTION-02** 迁移 soft visibility jobs；插件发布信任仍以 **K-SUPPLY-09** 为主要 P1。

...
| **K-DISTRO-01** | 内核打包资源化：内核旁迁移目录发现 + spawn env 注入 + Windows PATH 大小写修复 + bundle/e2e/resource 扩展 + `sql_migrate.rs` 迁移源 | **P1** | 保持 portable e2e 与发行打包门禁；`resources/migrations/` 由 bundle 脚本从内核 39 个迁移生成，仓库只跟踪占位文件 | **Done**（2026-08-14：`fdcf0db8`，PR [#155](https://github.com/linkaiheng2233-cyber/oclivenewnew/pull/155) 合入 `main` `0e6254d8`；PR CI 与主 CI [`31724393482`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31724393482) 均 16/16 success） |
| **K-VOICE-10** | 语音/门禁脚本：Windows stress 子进程树回收、异常路径清理、门禁超时与前端测试修正 | **P2** | 长时真实 GPU 矩阵仍归 K-VOICE-09；本项只保证短测/异常退出不会遗留进程 | **Done**（2026-08-14：`ca86624b`，PR [#155](https://github.com/linkaiheng2233-cyber/oclivenewnew/pull/155) 合入 `main` `0e6254d8`；PR CI 与主 CI [`31724393482`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31724393482) 均 16/16 success） |
| **K-EMO-01** | 英文词表扩充通道暂放（VADER MIT / AFINN Apache-2.0 参考），中文完成后处理 | P3 | 中文词表完成后扩充英文词表 | **OPEN**（2026-08-11 已登记） |
| **K-EMO-02** | 情绪引擎 A 阶段：中文词表 JSON 化 + 加权打分（`bb70cbbd` 落地）；B M1 已叠加（[EMO] 契约 + 删 policy hold + 删 post 词表 + 降级保持，切片 0–4 `60d90d5b`…`3cb6806b`） | P2 | 维护者应用内验收中间态行为（÷max 后 `low_confidence_hold_threshold` 失效属预期，`neutral_hold` 已随 M1 删除；降级模拟：断网/停 Ollama 后发消息应返回兜底回复且情绪保持上一轮）；验收通过后由维护者确认收口 | **已验收**（2026-08-12：切片 0–4 本地提交；LLM 错误注入测试 `main_llm_failure_returns_fallback_reply_and_keeps_previous_emotion` 落地 · host lib 454/454 · clippy/fmt 全绿；**应用内 playtest 通过**——`chat_messages` turn 23/24 `bot_emotion=happy`、`emotion_source=llm`、`emotion_labels=[joy,surprise]` 落库 + narrative hint 更新；**降级模拟已执行（2026-08-12，API 端到端）**——LLM 整体不可用 → 兜底回复 + `reply_is_fallback=true` + 情绪保持（正式库 happy 未动）+ 无 [EMO] 残渣 + 临时 DB 零污染） |
| **K-EMO-03** | `BuiltinKeywordComplexEmotionProvider` 保留为 remote/directory 后端构建失败与 fast 路径的降级出口（`complex_emotion.rs:36` 结构 / `:76` impl；`backend_registry.rs:61` / `co_present.rs:34` / `remote_plugin` adapter 多处引用，**非单引用**，v1.8 文档记载需修正） | P3 | 观察后续是否存在可收拢的重复词汇路径，再评估删除或重构为显式降级出口 | **OPEN**（2026-08-12 登记，本轮不动） |
| **K-EMO-04** | 内核选择/分发缺陷：Tauri dev bundled 子目录解析失败、manifest 缺少可比较的 `builtAt` / `gitCommit`、旧 bundled 内核可能落后于情绪切片 | **P2** | 保持 dev/resource 双路径、统一 runtime build identity 注入与 manifest 比较测试；发行重建仍须从目标提交生成 | **Done**（2026-08-14：`b9e1af3f`，dev `resources/` 兼容、runtime 统一构建身份、portable e2e 均落地；PR [#155](https://github.com/linkaiheng2233-cyber/oclivenewnew/pull/155) 16/16 success；merge `0e6254d8` exact-head 主 CI [`31724393482`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31724393482) 16/16 success） |
| **K-EMO-05** | 情绪趋势聚合：近 N 轮 `emotion_labels` 分布 → portrait/profile 慢变量（短期性格档案等未来消费方），依赖 M2 切片 4 决策包存档积累历史数据 | **P3** | 另立任务：M2 决策点 5 拍板，待趋势数据攒起来后再设计 | **OPEN**（2026-08-13 登记） |
| **K-EMO-06** | memory 情绪门槛候选增强：高情绪事件优先记忆（事件强度/情绪维度加权进记忆合并与长期记忆）；当前记忆链路不直写情绪列，情绪经 Event 六槽与好感度间接影响（M2 切片 1 已验证现状） | **P3** | 另立任务：M2 决策点 4 拍板仅验证现状，实现后续单独决策 | **OPEN**（2026-08-13 登记） |
| **K-ENCODING-01** | 中文编码事故治理：PS5.1 管道/US-ASCII 有损替换，4 起（K-EMO-01 词表乱码 · 台账 CJK 行 `00f6ea53` · M2 Plan 全 `?` · deepseek 蓝图锚点 `a4c664e3` 恢复 + `3fb6dd23` 按设定档案重写（v1.3.1））；现场复现：`$OutputEncoding=US-ASCII` 管道传中文 → `?` | P2 | 红线已落 `AI_AND_PIPELINE_GATES.md §7` + pipeline SKILL + AGENTS 易漏点；后续会话写中文文件须写后自查（汉字>0 / 无 `\?{3,}` / 无 BOM） | **OPEN**（2026-08-13 登记；红线落地，长期执行观察） |
...
| **V-MODULE-QUALITY-01** | “上限 = 模块上限”缺少组件质量对比 harness | **P1** | 固定角色/场景/replay 输入；可复现比较 memory / emotion / prompt / LLM 模块的质量与行为指标，而非只比延迟 | **Done**（[Full 计划](./debt-marathon/long-plans/V-MODULE-QUALITY-01.md)；Stage 1 `675561d5` 冻结 fixture/评分合同；Stage 2 `c883ceae` 真实 HTTP/replay/remote-slot 采集；Stage 3 `885b0b75` 两套显式配置、行为/性能分栏、双语文档与 Dimension 5 27/27；Stage 4 exact-head `4944fdf5` 主 CI [`31739849579`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31739849579) 16/16、严格审计 [`31739849550`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31739849550) success；PR [#156](https://github.com/linkaiheng2233-cyber/oclivenewnew/pull/156) 普通合并 `af82c758` 的主 CI [`31744282812`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31744282812) 16/16 success。两套配置是可复现合同基线，不冒充两套真实生产模型的主观质量结论） |
...
| **K-CI-IMPACT-01** | 单一全量 CI 不理解 OCLive 模块语义，重复构建且无法为新增模块解释验证范围 | **P1** | 版本化模块描述、中央影响图与受信验证目录；Rust 确定性规划器提供 `plan/explain`、未知范围全量回退。Stage 2 以实际结果度量漏选/过选；Stage 3 逐类开放低风险 PR，并由稳定 `ci-gate` 汇总，规划异常仍全量且门禁失败；模拟语料只做规则回归 | **In progress · Stage 3.1 draft selective；新 ready 类别仍 Compare**（既有真实对照：[`30692428026`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/30692428026) 为首个 21/21 样本；[`30708363964`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/30708363964) 绑定语音分段提交 `6164d6eb`，全量约 48 分 47 秒，未选 `cli` / `kernel-host-dual-core` / `rust-arm64-cross` 亦通过；纯文档 PR #159 的 [`31828405121`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31828405121) 16/16 成功并授权 ready docs Canary。2026-08-18：`domain-aware-pr-v2` 允许安全草稿 PR 按计划执行并使用非 required 的 `ci-draft-gate`；`ready_for_review` 对同一提交重新触发，除 docs Canary 外全量，Push 继续全量。每次全量 run 自动保留 90 天同 workflow SHA Compare 证据，选择性 run 永不标作权威对照；失败自动重跑改为人工 run ID + Rust allowlist。2026-08-21 复核发现一次 Dependabot 周期在同一分钟创建 **8** 条 ready PR，按安全策略各自触发全量，并非规划器失效；现改为 npm/Cargo 兼容更新分别分组、错峰、7 日冷却、每生态最多 3 条版本 PR，主版本迁移不再自动开 PR，Security updates 仍独立处理。模拟语料扩为 **21/21（18 targeted / 3 fail-safe）**，加入 Dependabot 节奏并保留中文/NUL、混合模块和 merge 策略；shared/role/plugin 前端影响环仍作为过选候选，不凭模拟删边。脚手架继续不拥有工作流、验证器、Runner、Secret、门禁或第二套解析器。实施 SSOT：[`SOMEDAY_TOOLCHAIN_CI.md`](../creator-docs/roadmap/SOMEDAY_TOOLCHAIN_CI.md)） |

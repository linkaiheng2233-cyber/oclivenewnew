...
| **K-EMO-04** | 内核选择/分发缺陷：① `distros/desktop-tauri/src/desktop_host.rs:30` `bundled_kernel_binary` 只 join `resource_dir` 根，Tauri dev 下实际资源在 `resources/` 子目录 → dev 模式 bundled 候选恒解析失败（2026-08-12 实测，spawn 落到 shared/dev）；② manifest `builtAt` 恒空（构建脚本未注入 `OCLIVE_KERNEL_BUILT_AT` / `OCLIVE_KERNEL_GIT_COMMIT`），`should_promote_binary` 无法按版本比较；③ 15:20 release/bundled 内核构建于切片 3 提交（15:20:55）之前、缺 emotion_source 落库 | **P2** | ① dev 路径兼容 `resources/` 子目录；② 构建脚本注入 builtAt/gitCommit（参考 `KernelBinaryManifest::from_compile_time_env`）；③ 已由本轮 release 重建（`3cb6806b`）手动替换 runtime + bundled 缓解，替换后 `--version` 冒烟通过 | **OPEN**（2026-08-12 登记；修复待 M2 前后排期） |

...
**Last updated:** 2026-07-15（K-VOICE-04 Minimal **Pending CI** · 下一动作跨仓 VSCODE）
...
| **K-VOICE-04** | 角色包可选 TTS 覆盖与全局配置冲突 | P3 | 全局 profile 必须服务所有角色；角色 `synth_profile` 只覆盖播报任务，不得在切换角色时改写设置 | **Pending CI**（`useRoleVoiceProfileSync` 已删除；`voiceTtsRouting` + AutoTts/Warm 任务级路由；build-directive 断言 override/inherit · 2026-07-15） |

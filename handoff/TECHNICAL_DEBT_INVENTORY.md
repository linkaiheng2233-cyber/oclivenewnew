...
| **K-CI-IMPACT-01** | 单一全量 CI 不理解 OCLive 模块语义，重复构建且无法为新增模块解释验证范围 | **P1** | Stage 1：版本化模块描述、中央影响图与受信验证目录；Rust 确定性规划器提供 `plan/explain`、未知范围全量回退；GitHub 影子 job 只产报告、不跳现有 job。Stage 2 以全量结果度量漏选/过选；证据成熟后才允许低风险 PR 选择性执行 | **In progress · Stage 1 Shadow 已实现**（2026-08-01：远端 CI [`30656699601`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/30656699601) **22/22** jobs success；现有 **14** 个仓内领域描述已包含独立 `oclive.scaffold`，契约改动靶向 Rust/CLI/Dimension5 并传播到 `oclive.cli`。影子 job 继续只积累漏选/过选证据，不跳现有 job；脚手架不拥有工作流、验证器、Runner、Secret、门禁或第二套解析器。实施 SSOT：[`SOMEDAY_TOOLCHAIN_CI.md`](../creator-docs/roadmap/SOMEDAY_TOOLCHAIN_CI.md)） |
...
| **K-LLM-ENV-02** | `apply_user_llm_env` 在 DB snapshot 读取后才取得进程环境锁，并把调用结束时的最新版本直接标记为已应用；并发旧调用可能覆盖新环境却清除 dirty 标记 | **P1** | 串行化完整的“读设置 → token/cache → env/provider → 版本提交”事务，或改为不可变配置快照；版本变化时必须重试而非误报已应用；用可控交错测试证明 last-writer-wins，覆盖保存设置、chat、theater 与 canonical sync 调用链 | **Locally verified**（2026-08-01：全事务改由单一异步 mutex 串行化；只提交实际读取的起始版本，期间若出现新版本则恢复 dirty，等待调用会继续应用；新增稳定快照/新版本保留 dirty 两项单测，Host 定向测试与 `-D warnings` Clippy 通过。待远端 CI 与后续并发压力证据后关闭） |
| **K-FRONTEND-TYPECHECK-01** | Vite/ESLint/Vitest 均不执行 Vue/TypeScript 类型检查，CI 可在真实分支错误存在时保持全绿 | **P1** | 引入 `vue-tsc` 与正确的 shared/chat-pro/theater project/alias 边界；先修零或建立只降不升 ratchet，再接 `check`/PR 硬门禁；为插件分享、协议安装与 Theater 大纲生成补行为测试 | **Locally verified**（2026-08-01：引入 `vue-tsc` 并接入本地 `check`、`check:release` 与 PR frontend job；修清 shared/Chat Pro/Theater 的真实诊断与跨发行版类型边界，修复插件分享分类、协议安装复核提示和 Theater cast 字段三条运行时路径并补行为测试。`typecheck`、lint、前端 **272** 条单测、Theater **53** 条测试、生产构建与 module compatibility 本地通过；待远端 CI 后关闭） |
| **D-CI-EXECUTION-02** | Stage 1 影子规划器尚未减少现有全量 job，通用 Rust job 又重复 CLI 嵌套 Cargo E2E；可见性 job 与硬门禁混居主 workflow | **P1** | 先按所有权去重 workspace/CLI/audit，串行或离线化嵌套 Cargo build 并稳定缓存；再把 soft visibility job 明确迁至 nightly/dispatch 或升为硬门禁；选择性执行仍须等待 Shadow 漏选/过选证据 | **Locally verified · ownership split**（2026-08-01：通用 Rust job 排除 `oclive-cli`，CLI job 独占串行嵌套 E2E 并启用 Rust cache；`cargo audit` 由 Dimension 5 唯一持有，`npm-audit` 升为 required；frontend job 明确执行 lint + `vue-tsc`。目录契约测试固定责任边界；本地非 CLI workspace 集成 **592.6s**、CLI 集成 **157.3s** 均通过。Stage 1 仍只 Shadow、不据此跳 job；待远端耗时证据后决定是否关闭本债） |

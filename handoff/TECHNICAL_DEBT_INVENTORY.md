...
| **K-ENCODING-01** | 中文编码事故治理：PS5.1 管道/US-ASCII 有损替换，4 起（K-EMO-01 词表乱码 · 台账 CJK 行 `00f6ea53` · M2 Plan 全 `?` · deepseek 蓝图锚点 `a4c664e3` 修复）；现场复现：`$OutputEncoding=US-ASCII` 管道传中文 → `?` | P2 | 红线已落 `AI_AND_PIPELINE_GATES.md §7` + pipeline SKILL + AGENTS 易漏点；后续会话写中文文件须写后自查（汉字>0 / 无 `\?{3,}` / 无 BOM） | **OPEN**（2026-08-13 登记；红线落地，长期执行观察） |

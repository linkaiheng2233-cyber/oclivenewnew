...
**Last updated:** 2026-07-29（Stable v4 能力计划已远端验证；统一资源协调 Resource Adapter Registry 里程碑进入本地收口）
...
| **K-RESOURCE-COORD-01** | llama-server / Ollama / CosyVoice 与未来 Live2D 各自决策资源，缺少统一预算和租约 | **P1** | 内部 `ExecutionPlan` + Resource Coordinator；LLM/Voice 两适配器先闭环 snapshot/admission/lease/pressure/degrade/release；覆盖不可控外部进程、取消、故障恢复与真实共享显存压力测，再接第三类渲染适配器 | **Partial · Stage 2.3 locally verified**（2026-07-30：Performance 门禁已从 fallback 扩展为完整请求生命周期，资源切换排空在途 primary/fallback 后再释放；显式恢复等待门禁开放且拒绝越过 Voice 驻留，所有权移交前取消会自动撤销暂停。官方 bundled `voice.speak` 在本地 provider、显存余量拒绝、无活动 LLM 且存在可控驻留时执行有限抢占；Chat Pro 将生成期间拒绝延后至最终文本 RPC，插件确认 CosyVoice 卸载后才恢复 LLM，移交后的未确认或取消则保留/提升租约并暴露稳定原因；bundled 直连端点仅在宿主准入 warm 后暴露。定点 `performance_llm` **13** 测、`resource_coordination` **8** 测、共享语音 **14** 测、host lib Clippy、Voice ratchet 与 module-compat 均通过。当前仍为 `coordinator_selectable=false`；仍缺通用公平队列/多档切换、第三方入口、不可控外部进程故障矩阵、真实共享显存 stress/soak、RAM/CPU 与第三类渲染适配器；未升 Done） |

...
**下一动作：** 先完成 **V-MODULE-QUALITY-01 Stage 3–4** 与依赖/CI 运行时更新；**K-VOICE-09** 的 30 分钟真实矩阵、K-RESOURCE-COORD-01 长时硬件 soak 和人工听感按维护者决定延后到新电脑。随后进行 **R18 配置审计与维护者确认测试**，再按愿景验证顺序推进 V-PORTABLE-01 Full → V-EMBED-01。发行版仍需 **V-VSCODE-PERF-05** 的姊妹仓 F5 / `.vsix` 实机证据；**K-PLUGIN-SEC-01、K-SUPPLY-09、K-SUPPLY-10 仍须保持显式 OPEN / Partial，不因排期后移而降格或消失。**
...
| **K-DISTRO-01** | 内核打包资源化：内核旁迁移目录发现 + spawn env 注入 + Windows PATH 大小写修复 + bundle/e2e/resource 扩展 + `sql_migrate.rs` 迁移源 | **P1** | 保持 portable e2e 与发行打包门禁；`resources/migrations/` 由 bundle 脚本从内核 39 个迁移生成，仓库只跟踪占位文件 | **Done-eligible · merged**（2026-08-14：`fdcf0db8`，PR [#155](https://github.com/linkaiheng2233-cyber/oclivenewnew/pull/155) 合入 `main` `0e6254d8`；PR CI 16/16 success；主 CI [`31724393482`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31724393482) 尚待 Windows Rust 收尾） |
| **K-VOICE-10** | 语音/门禁脚本：Windows stress 子进程树回收、异常路径清理、门禁超时与前端测试修正 | **P2** | 长时真实 GPU 矩阵仍归 K-VOICE-09；本项只保证短测/异常退出不会遗留进程 | **Done-eligible · merged**（2026-08-14：`ca86624b`，PR [#155](https://github.com/linkaiheng2233-cyber/oclivenewnew/pull/155) 合入 `main` `0e6254d8`；PR CI 16/16 success；主 CI 收尾中） |
...
| **V-MODULE-QUALITY-01** | “上限 = 模块上限”缺少组件质量对比 harness | **P1** | 固定角色/场景/replay 输入；可复现比较 memory / emotion / prompt / LLM 模块的质量与行为指标，而非只比延迟 | **Done-eligible**（[Full 计划](./debt-marathon/long-plans/V-MODULE-QUALITY-01.md)；Stage 1 `675561d5` 冻结 fixture/评分合同；Stage 2 `c883ceae` 真实 HTTP/replay/remote-slot 采集；Stage 3 `885b0b75` 两套显式配置、行为/性能分栏、双语文档与 Dimension 5 27/27；Stage 4 exact-head `4944fdf5` 主 CI [`31739849579`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31739849579) 16/16、严格审计 [`31739849550`](https://github.com/linkaiheng2233-cyber/oclivenewnew/actions/runs/31739849550) success。两套配置是可复现合同基线，不冒充两套真实生产模型的主观质量结论） |

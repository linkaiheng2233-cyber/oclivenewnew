...
| **K-EMO-02** | ???? A ??????? JSON ? + ?????`bb70cbbd` ????B M1 ????[EMO] ?? + ? policy hold + ? post ?? + ??????? 0?4 `60d90d5b`?`3cb6806b`? | P2 | ???????????????max ? `low_confidence_hold_threshold` ??????`neutral_hold` ?? M1 ??????????/? Ollama ?????????????????????????????????? | **???**?2026-08-12??? 0?4 ?????LLM ?????? `main_llm_failure_returns_fallback_reply_and_keeps_previous_emotion` ?? ? host lib 454/454 ? clippy/fmt ???**??? playtest ??**??`chat_messages` turn 23/24 `bot_emotion=happy`?`emotion_source=llm`?`emotion_labels=[joy,surprise]` ?? + narrative hint ???**????????2026-08-12?API ????**??LLM ????? ? ???? + `reply_is_fallback=true` + ???????? happy ???+ ? [EMO] ????? DB ???? |

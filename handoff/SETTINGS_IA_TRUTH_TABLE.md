...
与 [`SETTINGS_NAV_KEYS`](../src/lib/settingsNavKeys.ts) 中的 **稳定菜单 id** 对齐；L 级含义见 [`settingsNavCopy`](../src/lib/settingsNavCopy.ts) 引用的 `settings.tiers.*` 文案。
...
从设置发起深链（本机模型、插件管理、市场、专家工作台、调试）时 **先关闭设置浮层**，再打开目标面板；与既有「从云端区块打开后端」行为一致。

...
Resource Coordinator 已落地 NVIDIA 多设备 snapshot、admission、lease、priority、pressure 与诊断；宿主 Resource Adapter Registry 进一步登记 managed llama-server、observe-only Ollama、performance 活动观察器和官方 bundled CosyVoice2 的控制权、运行档位、驻留能力与真实生命周期动作，租约携带对应 `profile_id`。bundled CosyVoice 配置退出采用“请求卸载 → 侧车确认 → 撤销租约”，未确认时保留租约并暴露稳定原因码；配置转换与 warm/speak 共用适配器串行锁。Performance LLM 资源暂停已具备统一 fallback 门禁：先拒绝新 Ollama fallback、排空已进入请求，再卸载本运行时追踪的模型；所有生成入口与启动探针同门，显式预热/模型应用恢复。纯计划编译/CLI 不碰硬件，保留 `not_evaluated`；桌面诊断刷新为 `ready | degraded | blocked` 并返回适配器目录。云/社区 TTS 不被误管，token/PCM 仍走原数据通道。当前档位均未声明为协调器可自动选择；Voice 尚未接入自动暂停、失败回滚和释放后恢复。尚未落地的是第三方注册入口、公平队列/自动抢占恢复、真正的多档切换、RAM/CPU 与渲染适配器；状态见 `K-RESOURCE-COORD-01`。

实现锚点：[`execution_plan.rs`](../kernel/crates/oclive_kernel_host/src/domain/execution_plan.rs)（纯编译）· [`capability_registry.rs`](../kernel/crates/oclive_kernel_host/src/infrastructure/capability_registry.rs)（能力适配）· [`resource_adapter_registry.rs`](../kernel/crates/oclive_kernel_host/src/domain/resource_adapter_registry.rs) / [`resource_coordinator.rs`](../kernel/crates/oclive_kernel_host/src/domain/resource_coordinator.rs)（资源目录与决策）· [`performance_fallback_gate.rs`](../kernel/crates/oclive_kernel_host/src/infrastructure/performance_fallback_gate.rs)（Performance LLM fallback 准入/排空/恢复竞态）· [`service/execution_plan.rs`](../kernel/crates/oclive_kernel_host/src/service/execution_plan.rs)（激活门禁/只读查询）· [`models/execution_plan.rs`](../kernel/crates/oclive_kernel_types/src/models/execution_plan.rs) / [`models/resource_coordination.rs`](../kernel/crates/oclive_kernel_types/src/models/resource_coordination.rs)（公共诊断 DTO）。

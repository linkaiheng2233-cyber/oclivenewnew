...
| **`npm audit`** | `.github/workflows/ci.yml` 的 `npm-audit` 双硬门禁 | 依次验证生产图 `npm audit --omit=dev --audit-level=high` 与完整开发图 `npm audit --audit-level=high`；`npm ls` 用于核对 peer 关系，不能用生产 0 代替完整依赖图证据。 |

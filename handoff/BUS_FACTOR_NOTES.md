...
| 环节 | 文件/类型 | 说明 |
|------|-----------|------|
| **核心错误枚举** | `oclive_kernel_runtime` 的 **`AppError`**（宿主 `src-tauri/src/error.rs` 再导出） | 业务语义与 `thiserror` 变体。 |
...
| **宿主桥** | `src-tauri/src/error.rs` 的 **`to_invoke_error`** | 将内核错误转为 `InvokeError`。 |
| **目录插件 ApiError** | `src-tauri/src/api/error.rs` | 与内核 `code` 对齐。 |
| **前端 i18n** | `src/i18n/locales/fragments/apiErrors.*` + `toFriendlyErrorMessage` 等 | 机器码 → 用户可读文案；新增码需 **中英** 词条 + [`ERROR_CODES.md`](../creator-docs/getting-started/ERROR_CODES.md) 速查表。 |
...
| **脚手架** | **`crates/oclive-cli/`**：`init --monolith` 生成 **`monolith.toml`**、**`vendor/oclive_monolith_builtin/`**、**`process_message_monolith.rs`**、双 **`[[bin]]`**。 |
...
| **manifest 与 README** | 根目录 **`roles/README_MANIFEST.md`**。 |
| **校验 crate** | **`crates/oclive_validation`**：与宿主、编写器共享；顶层键等见 crate 内 `json_keys` / manifest 校验模块。 |
| **编写器 wasm** | **oclive-pack-editor**：`npm run wasm:build` 指向相邻克隆的 **`oclivenewnew/crates/oclive_validation`**，详见该仓 **README / CONTRIBUTING**；未构建 wasm 时 TypeScript 子集回退。 |
...
| **`invoke` 热路径** | **`src-tauri/tests/invoke_hotpath_matrix.rs`**；矩阵 **`handoff/INVOKE_HOTPATH_MATRIX.md`**。 |

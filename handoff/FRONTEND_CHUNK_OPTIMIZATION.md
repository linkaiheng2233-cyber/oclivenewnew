...
- **`App.vue`**：`PluginManagerV2Panel` 使用 `defineAsyncComponent` + **`v-if="pluginManagerV2Open"`**，首次打开插件管理 V2 再拉取该路由级 chunk。
- **`PluginManagerV2` / `ExpertModelsPanel`**：`defineAsyncComponent` 懒加载专家模型与画布（`@vue-flow`）。
- **`vite.config.js`**：`vendor-vue-flow`；不对 `@sentry` 做 `manualChunks`（避免空 chunk）。

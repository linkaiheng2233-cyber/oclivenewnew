...
| **配置专家模型设施子模块**（专家路由 · 机制预埋、UI 暂未挂载） | 见下「配置专家模型」；**勿**在编写器 `.oclexpert` 实验页新建路由 |
...
2. **专家路由 UI（架构图节点）暂未挂载**；运行时机制已预埋（`dual_core` feature，默认不编译）。编辑 **`blueprint/includes/expert_routing.json`**（必要时在蓝图 `includes[]` 中引用），或经 Tauri `get_expert_routing` / `save_expert_routing` API。
3. 若之后在编写器中修改人设并保存，编写器会**保留** `includes` / `groups` 等扩展字段；专家路由文件本身不会被编写器删除。
4. 编写器高级页中的 **`expert/default.oclexpert`** 为遗留实验格式，**不是**运行时专家路由。

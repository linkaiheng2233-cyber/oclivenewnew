...
oclive 的长期方向，是在 **本地优先、可替换子系统、角色包为唯一对接面** 的前提下，把运行时做成 **创作者与玩家都能安全实验** 的桌面底座：模块可切换（内置 / v2 / 远程侧车）、契约与 CI 守住兼容边界，文档与示例降低接入成本。架构上见 **[单核双态构建架构](../getting-started/OCLIVE_ARCHITECTURE_OVERVIEW.md)**（外核态可替换 + 可选宏核态 Monolith 焊接）。

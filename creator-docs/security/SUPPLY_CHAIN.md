...
| **K-SUPPLY-04** | `npm-audit` 升格 | P2 | **Locally verified** — 两周期生产扫描均为 0，`continue-on-error` 已移除；待远端硬门禁证据后关闭 |
...
| **K-SUPPLY-11** | `event-listener` 5.4.1 unsound warning | P1 | **Locally verified** — 锁文件已升级 5.4.2，SQLx 与 zbus/Tauri 均解析到修复版；`cargo audit` 警告 9→8，待远端证据后关闭 |
...
4. **`npm-audit`**：已作为生产依赖高危硬门禁运行；本地零命中，关闭技术债前仍需记录一次远端成功证据。

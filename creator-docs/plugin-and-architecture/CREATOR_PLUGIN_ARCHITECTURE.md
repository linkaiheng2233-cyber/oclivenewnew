...
宿主还会在 **`event.estimate`** 与 **`prompt.build_prompt`** 的 `params` 中附带字符串 **`personality_source`**（`vector`|`profile`）；侧车可按需读取，忽略则与旧版兼容（见协议 §3.4）。
...
| 更新角色台词、场景、核心档案 | ✅ | 更新角色包文件 + 用户 **`load_role`** |

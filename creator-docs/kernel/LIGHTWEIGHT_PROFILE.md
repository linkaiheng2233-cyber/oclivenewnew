...
| **`default-event-providers`** | **官方默认事件模块（薄壳）**：**`oclive_event_builtin`** 提供进程内 **`BuiltinEventEstimator*`**，经 **`EventImpactEngine`** 委托运行时 **`event_impact_ai`**（算法仍在 `oclive_kernel_runtime`）；关闭后回退 `Ignore` 桩。目录形态示例：**`examples/oclive-event-builtin-directory/`**（`event.estimate`，需 **`process:spawn`**）。依赖面历史评估见 [`KERNEL_V2_DESIGN.md`](./KERNEL_V2_DESIGN.md) §6。 |

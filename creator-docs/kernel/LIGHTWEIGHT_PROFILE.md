...
| **`default-emotion-providers`** | 进程内 **`UserEmotionAnalyzer` Builtin / BuiltinV2**（设施 crate **`oclive_emotion_builtin`** 的 `providers`）；关闭后回退中性分布桩。**算法库** `oclive_emotion_builtin::classic`（`EmotionAnalyzer`）仍被 `emotion_analyzer` / `EmotionResultExt` 与 Remote HTTP 回退路径依赖。恢复 **directory** 形态：示例 **`examples/oclive-emotion-builtin-directory/`**（`com.oclive.builtin.emotion`，`emotion.analyze`），需 **`process:spawn`**。 |
| **`default-event-providers`** | 内置事件影响估计（`event_impact_ai` / `EventEstimator`）；关闭后回退 `Ignore` 桩。**设施 crate 剥离划为阶段 6**（依赖面见 [`KERNEL_V2_DESIGN.md`](./KERNEL_V2_DESIGN.md) §6）。 |
| **`default-prompt-providers`** | 内置 `PromptBuilder` / `PromptAssembler`；关闭后回退空串桩。**设施 crate 剥离划为阶段 6**（同上 §6）。 |

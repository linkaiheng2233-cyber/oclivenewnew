...
> **命令 ↔ `api/` / 内核模块对照表**：[KERNEL_API_IMPLEMENTATION_MATRIX.md](./KERNEL_API_IMPLEMENTATION_MATRIX.md)（含 **纯转发 / 壳层增强** 分层说明）  
> **轻量场景与 `Cargo` 特性 / `invoke` 分组**：[LIGHTWEIGHT_PROFILE.md](./LIGHTWEIGHT_PROFILE.md)  
> **嵌入 SDK（`KernelAppState` / `kernel_server`）**：[KERNEL_SDK.md](./KERNEL_SDK.md)

...
2. **行为与偏好单页**：侧栏「模型·语言·通知」对应 `settings.general.behavior`，右栏以多张卡片拆分默认模型、语言区域与通知说明；快捷键绑定仍在侧栏「快捷键管理」。
3. **角色与数据**：「角色管理」页内并列 **专家模型** 摘要卡，可一键进入工作台（需总闸开启门控页时会有提示）。
4. **系统与内核总览**：沉浸下侧栏「系统与内核总览」按子域列出常用高级入口的跳转按钮，减少在长侧栏中逐项查找。
5. **设置中心开发者总闸**（仅沉浸、设置窗顶部）：关闭时侧栏隐藏 V2 专用项（专家模型工作台、V2 插件管理 / 市场、实验开关页、市场索引开发者区、Agent 调试说明等），避免日常用户误入高影响路径；开启后上述入口出现，且右栏顶部会显示 **高级功能** 提示条与 **返回常规设置**。
6. **侧栏分组提示**：分组标题旁的小帮助图标可一眼看到该组大致包含语言/快捷键、模型、角色数据、插件、实验、系统诊断等哪类内容。
7. **L1–L4 徽章**：侧栏每项旁的缩写与右栏 `SettingsTierSection` 标题对应风险级别；展开 **L4** 区块前通常需二次确认。

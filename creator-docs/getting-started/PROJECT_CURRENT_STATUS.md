...
- **运行时**：Tauri 桌面端；角色包导入（`.ocpak` / `.zip` / 目录）、对话主路径 **`process_message`**、六槽 `plugin_backends`、目录插件、Remote 侧车、本地 HTTP `--api`、启动健康检查等（细节见 [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md)）。

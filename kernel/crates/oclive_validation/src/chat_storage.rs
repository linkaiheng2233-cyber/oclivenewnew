...
pub fn validate_chat_storage_config(value: &serde_json::Value) -> Vec<String> {
    ...
    for key in [
        ...
        if let Some(v) = value.get(key) {
            match v.as_u64() {
                Some(n) if n >= 1 => {}
                _ => errs.push(format!("config.json chat_storage.{key} 须为正整数")),
            }
        }
    }
...
pub fn validate_chat_storage_config_file(config_path: &std::path::Path) -> Result<(), Vec<String>> {
...
mod tests {
    ...
    fn rejects_threshold_out_of_range() {
        ...
        assert!(errs
            .iter()
            .any(|e| e.contains("replay_similarity_threshold")));
    }

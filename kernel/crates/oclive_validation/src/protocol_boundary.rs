//! Layered validation of JSON-RPC sidecar errors vs kernel [`KernelErrorBody`] (AB2).
//!
//! Kernel body fields are `code` / `message` / `hint` (see `oclive_kernel_runtime`), **not** `detail`.
...
/// Validates JSON-RPC 2.0 error object: `error.code` is an **integer**, `error.message` is a string.
///
/// # Errors
///
/// Returns [`ProtocolValidationError`] when shape or types do not match JSON-RPC 2.0 error object.
...
/// Validates kernel HTTP / Tauri shared `KernelErrorBody` shape: `code` is a **SCREAMING_SNAKE_CASE** string.
///
/// # Errors
///
/// Returns [`ProtocolValidationError`] on missing fields, type errors, or mixing `code` with JSON-RPC integer form.
...
/// Sidecar integer `code` and kernel string `code` must not share the same field shape.
///
/// # Errors
///
/// Returns the corresponding [`ProtocolValidationError`] when either layer fails validation.

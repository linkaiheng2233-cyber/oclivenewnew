...
use crate::infrastructure::chat_storage::{
    delete_mirror_scene_dir, delete_mirror_tree_for_role, load_max_messages_per_session,
    load_role_chat_storage_root, load_storage_root, pick_export_max_messages,
...
pub async fn execute_chat_storage_proxy(
    ...
    match op {
        ...
        ChatStorageProxyOp::SearchMessages {
            ...
            let hits = state
                .conversation_store
                .search_messages(query.trim(), role_id.as_deref().map(str::trim), cap, offset)
                .await?;
            Ok(serde_json::to_value(hits).map_err(|e| {
                AppError::InvalidParameter(format!("chat storage proxy encode: {e}"))
            })?)
        }
        ...
        ChatStorageProxyOp::ExportSession { session_id, format } => {
            let session_id = session_id.trim().to_string();
            ...
            let res = state
                .conversation_store
                .export_session(&session_id, format.trim(), max, role_name.as_deref())
                .await?;
            Ok(serde_json::to_value(res).map_err(|e| {
                AppError::InvalidParameter(format!("chat storage proxy encode: {e}"))
            })?)
        ...
        ChatStorageProxyOp::ReplayProgress { task_id } => {
            let progress = state.replay_tasks.get(task_id.trim()).ok_or_else(|| {
                AppError::InvalidParameter(format!("replay task not found: {}", task_id.trim()))
            })?;
            Ok(serde_json::to_value(progress).map_err(|e| {
                AppError::InvalidParameter(format!("chat storage proxy encode: {e}"))
            })?)
        }
        ChatStorageProxyOp::Capabilities => {
            ...
            let caps = ChatStorageCapabilities {
                ...
                default_max_messages_per_session:
                    crate::infrastructure::chat_storage::DEFAULT_MAX_MESSAGES as u32,
            ...
            Ok(serde_json::to_value(caps).map_err(|e| {
                AppError::InvalidParameter(format!("chat storage proxy encode: {e}"))
            })?)
        }
        ChatStorageProxyOp::DeleteRoleChats { role_id } => {
            ...
            state
                .db_manager
                .delete_chat_data_for_manifest_role(rid)
                .await?;
        ...
        ChatStorageProxyOp::DeleteSceneChats { role_id, scene_id } => {
            ...
            let sessions_deleted = state
                .db_manager
                .delete_chat_data_for_role_scene(rid, sid)
                .await?;
            let res = DeleteChatsResult {
                sessions_deleted,
                bytes_freed: bytes,
        ...
        ChatStorageProxyOp::GetStorageRoot => {
            ...
            let root = load_storage_root(app_data).to_string_lossy().into_owned();

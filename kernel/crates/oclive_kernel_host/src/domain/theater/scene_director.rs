...
pub async fn generate_scene(
    ...
    let max_beats = req.max_beats.unwrap_or_else(ripple_max_beats).clamp(4, 64);
...
fn fallback_response(req: &TheaterSceneRequest, model: &str, source: &str) -> TheaterSceneResponse {
...
async fn generate_cast_adapt_scene(
    ...
    let max_beats = req
        .max_beats
        .unwrap_or_else(|| (req.base_beats.len() as u32).max(ripple_max_beats()))
        .clamp(4, 64);
    let (llm, model) = match resolve_theater_llm(state, req).await {
        Ok(v) => v,
        Err(e) => {
...
fn parse_cast_rewrite_beats_loose(
    ...
    parse_cast_rewrite_beats_loose_with_ctx(
        raw,
        min_beats,
        max_beats,
        &CastRewriteParseCtx {
...
fn parse_rewrite_line_object(
    v: &Value,
    ctx: Option<&CastRewriteParseCtx>,
) -> Option<TheaterScriptLine> {
    let cast_raw = v
        .get("cast")
...
fn merge_adapted_line(base: &TheaterScriptLine, adapted: &TheaterScriptLine) -> TheaterScriptLine {
...
pub(crate) async fn resolve_theater_llm(
    ...
    let role = if let Ok(role) = state
        .load_role_cached_async(req.cast_a.role_id.trim())
        .await
    {
        role
    } else if let Ok(role) = state
        .load_role_cached_async(req.cast_b.role_id.trim())
        .await
    {
        role
    } else {
    ...
    let model = resolve_effective_ollama_model(state, role.as_ref(), session_ns.as_str()).await?;
    Ok((llm, model))
}
...
pub(crate) fn ripple_ids_conflict(
    prefix: &[TheaterScriptLine],
    ripple: &[TheaterScriptLine],
) -> bool {
...
pub(crate) fn build_scene_prompt(
    ...
    let scope_block = if ctx.full_rewrite {
        format!("无微调：重写整场（≤{max_beats} 拍）。开场骨架：\n{ripple_json}")
    } else {
        format!(
...
mod tests {
    use super::*;
    use crate::models::dto::{
        TheaterCastRef, TheaterForkTemplate, TheaterPokeChipDef, TheaterTweak,
    };

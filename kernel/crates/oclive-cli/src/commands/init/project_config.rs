...
pub struct ProjectConfig {
    ...
    /// Which sample role pack to generate (`None` = do not create `roles/`).
    pub role_pack_kind: RolePackKind,
    /// Generate `monolith.toml` and Monolith build config only when `kernel_server` and true.
    pub monolith_enabled: bool,
    /// Monolith weld preset; used for the `monolith.toml` generated by init only when `monolith_enabled`.
    pub monolith_preset: Option<MonolithPresetArg>,
    /// When true, do not generate the `roles/` directory (blank kernel template).
    pub skip_role_pack: bool,
    /// Copy the llamacpp sample directory plugin into `plugins/`.
    pub with_example_plugin: bool,
    /// Kernel factory template to use (generates artifacts such as robot-gateway MCP).
    pub factory_template: Option<InitTemplateArg>,
    /// Automatically run the Monolith benchmark after generation completes.
    pub run_monolith_bench_after_init: bool,
    /// Points to the oclivenewnew repo root; the generated project writes path dependencies and replaces the placeholder `main`/`lib`.
    pub kernel_source: Option<std::path::PathBuf>,
    /// Generates `[package].authors` in `Cargo.toml`.
    pub cargo_author: Option<String>,
    /// Generates `[package].license` in `Cargo.toml` (defaults to MIT).
    pub cargo_license: Option<String>,
    /// Generates `[package].description` in `Cargo.toml`.
    pub cargo_description: Option<String>,
    /// Orchestration mode (generates `docs/PIPELINE_CUSTOM.md` and `src/oclive_pipeline_order.rs`).
    pub pipeline: crate::pipeline::PipelineArg,
    /// Custom `weld_modules` for `monolith.toml` (TUI or `--weld-modules`).
    pub custom_weld_modules: Option<Vec<String>>,
    /// The sample role pack writes a v3 blueprint and enables `runtime_config.dual_core.enabled`.
    pub dual_core_enabled: bool,
    /// Chat history storage backend (written to role pack `config.json` → `chat_storage.backend`).
    #[serde(default)]
    pub chat_storage_backend: ChatStorageBackend,
    /// Chat history storage location (`role_pack` follows the role pack, `global` is the global location).

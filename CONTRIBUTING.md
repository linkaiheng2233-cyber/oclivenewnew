...
**CI 对齐（重要）**：**`npm run check:release`** 已链 **`npm run test:unit`** 与 **`npm run verify:ui`**（见根 `package.json`）；**Playwright（`npm run test:e2e:preview`）不在 `check:release` 内**，仅在 Ubuntu **`frontend`** job 执行（见 `.github/workflows/ci.yml`）。有前端改动时，在 **Linux/macOS** 可另跑 **`npm run build && npm run test:e2e:preview`**，或确认 **Actions → frontend（ubuntu）** 已绿。当前发版勾选以 [PROJECT_OVERVIEW.md §8](creator-docs/getting-started/PROJECT_OVERVIEW.md#8-发版前可勾选的极简清单) 与 CI 为准。
...
3. **审阅**：至少一名维护者确认 **兼容层与迁移路径**、CI 与 [PROJECT_OVERVIEW.md §8](creator-docs/getting-started/PROJECT_OVERVIEW.md#8-发版前可勾选的极简清单) 的发版项。
...
- **发版与兼容**：semver bump 或契约变更时，核对 [`creator-docs/COMPATIBILITY.md`](creator-docs/COMPATIBILITY.md) 快照与一页表，并按 [PROJECT_OVERVIEW.md §8](creator-docs/getting-started/PROJECT_OVERVIEW.md#8-发版前可勾选的极简清单) 更新对外说明；角色包版本规则见 [PACK_VERSIONING.md](creator-docs/role-pack/PACK_VERSIONING.md)。

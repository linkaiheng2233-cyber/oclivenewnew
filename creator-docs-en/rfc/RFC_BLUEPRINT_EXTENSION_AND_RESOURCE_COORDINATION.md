...
- Existing profiles remain `coordinator_selectable=false`; general automatic switching is not claimed. Official bundled CosyVoice foreground speech now has a bounded low-headroom transition that can suspend the local Performance LLM and recover it after confirmed release. Third-party registration, fair queues, arbitrary-adapter preemption, real multi-profile transitions, RAM/CPU, render adapters, and shared-VRAM stress/soak remain future work.
...
- This slice does not kill an active LLM request to load voice and does not claim control over arbitrary external Ollama processes. General preemption, fair queues, and shared-VRAM stress/soak remain debt.

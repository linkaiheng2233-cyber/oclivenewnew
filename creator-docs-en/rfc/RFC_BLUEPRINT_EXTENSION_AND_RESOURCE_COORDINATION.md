...
- Existing profiles remain `coordinator_selectable=false`; automatic switching is not claimed. A coordinated Performance LLM fallback gate is implemented, while third-party registration, fair queues/automatic preemption and recovery, real multi-profile transitions, RAM/CPU, render adapters, and shared-VRAM stress/soak remain future work.
...
- Performance LLM now has a coordinated fallback gate. A resource transition rejects new Ollama fallbacks, drains already admitted fallback calls, and then unloads only the Ollama models tracked by this OCLive runtime. Ordinary primary failures and cloud-provider suspension retain their previous fallback behavior. `generate`, tag generation, opts, streaming, and startup probes all use the same gate. A concurrent explicit warm-up/runtime-selection request is recorded as a superseding recovery, but the gate does not reopen before draining and unload complete.
- Automatic llama-server → Voice preemption remains disabled: the gate is only the safety prerequisite. Voice admission does not yet initiate managed-llama suspension, roll back failures, or recover it after Voice release. Cancellation conservatively leaves the gate closed, so later orchestration must recover explicitly rather than rely on a timeout.

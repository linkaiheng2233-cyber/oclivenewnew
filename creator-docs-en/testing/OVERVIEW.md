...
|-------|---------|---------------------|
| Rust unit and integration tests | Orchestration, `--api` HTTP routes, `process_message`, **`invoke` hot path (nine `*_impl` chains)** ([`invoke_hotpath_matrix.rs`](../../distros/desktop-tauri/tests/invoke_hotpath_matrix.rs), see [`handoff/INVOKE_HOTPATH_MATRIX.md`](../../handoff/INVOKE_HOTPATH_MATRIX.md)), etc. | `cargo test` under `distros/desktop-tauri/`; integration tests in `distros/desktop-tauri/tests/` |
...
| Frontend smoke | Vitest guard + **`vite preview` + Playwright** shell (**A1.1b**; **CI: Ubuntu `frontend` only**) | `npm run test:unit`; `npm run build && npm run test:e2e:preview` ([`distros/chat-pro/e2e/preview-shell.spec.ts`](../../distros/chat-pro/e2e/preview-shell.spec.ts); see CONTRIBUTING **Windows** note) |
...
| **T14–T20** (`official-vue-test-runner`, etc.) | Editor-built capability, wired to the workspace as a **directory plugin** pattern; **T14 Vue runner** is in-repo at `distros/chat-pro/plugins/official-vue-test-runner/` (see plugin README). |

...
- **Smoke example**: [`distros/desktop-tauri/tests/plugin_backends_v2_resolve.rs`](../../distros/desktop-tauri/tests/plugin_backends_v2_resolve.rs)  
...
- Use **`MockLlmClient`** (`kernel/crates/oclive_kernel_host/src/infrastructure/`) implementing **`LlmClient`** so integration tests do not hit real Ollama.

## Directory / remote plugins

- **Remote**: see CI job **`remote-plugin-demo`** (minimal Python sidecar + `memory.rank` JSON-RPC).
- **Directory**: needs `DirectoryPluginRuntime` and on-disk `distros/chat-pro/plugins/` layout; higher integration cost—prefer **unit tests + smoke** layering.
...
[中文](../../creator-docs/testing/ADAPTING_TEST_PLUGIN.md)

...
| **K-SUPPLY-04** | Elevate `npm-audit` | P2 | **Ready to close** — production scans were zero on 2026-07-18 and 2026-08-01; remove `continue-on-error` and collect remote evidence |
...
| **K-SUPPLY-11** | `event-listener` 5.4.1 unsound warning | P1 | **OPEN** — reachable through both SQLx and zbus/Tauri; upgrade first or document reachability rather than silently ignoring it |
...
4. **`npm-audit`**: the two-cycle zero-production-vulnerability evidence is complete; current `continue-on-error: true` is the remaining implementation gap and should become a hard gate.

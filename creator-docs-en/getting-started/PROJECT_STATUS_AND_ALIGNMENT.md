...
- **Product launch**: still driven by gap checklist **§A**, decoupled as in checklist **§D**. **A1.1a** (HTTP process restart) and **A1.2** (`invoke` host hot path, nine `*_impl` chains — [`INVOKE_HOTPATH_MATRIX.md`](../../handoff/INVOKE_HOTPATH_MATRIX.md)) are landed; **default next engineering focus** is **A1.1b** (GUI / installer) and the rest of [PRODUCT_LINE_TASK_BUCKETS.md](../../handoff/PRODUCT_LINE_TASK_BUCKETS.md) **§四 (Hard items)** — **one issue per item**.

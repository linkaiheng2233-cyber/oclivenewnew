...
The v1 object requires `character_is_adult: true`; `pacing.mode` is `creator` or `ai`, `suggested_interval_ms` is an integer from **500 through 60000**, and every `scenes` key must reference a base scene id. Chat Pro injects it only after a local adult self-declaration plus the global and per-role gates; no online identity check is required. The declaration persists, but Settings must provide a reset action that first stops active adult generation and then restores all local R18 gates, role choices, pacing, and queue defaults without deleting conversations or memories. Legacy user-identity `adult_eligible` metadata remains readable for pack compatibility and creator guidance, but is not a hidden fourth authorization gate. `SendMessageResponse.schema` **16** returns `adult_beat` with dialogue, silent narration, interaction state, and an optional interval while retaining dialogue in the base `reply` compatibility field. Malformed envelopes use a prompt-safe fallback, and model-suggested intervals are clamped to 500–60000 ms.

When a present `adult_extension.json` cannot be read, parsed, or validated, the host isolates and disables only that extension, records a diagnostic, and exposes a prominent error to Chat Pro. The base role must remain listed and loadable in ordinary mode; an extension failure must not take down the role or the role list.

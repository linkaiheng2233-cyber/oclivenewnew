//! Chat log export for bridge and Tauri invoke.
...
use crate::service::role::slot_session::build_plugin_resolution_debug_info;
...
pub async fn export_chat_logs_impl(
    ...
    let role = state.load_role_cached_async(rid).await?;
    ...
    let plugin_debug = if include_plugin_debug {
        Some(
            build_plugin_resolution_debug_info(state, rid, req.session_id.as_deref()).await?,
        )
    } else {
        None
    };

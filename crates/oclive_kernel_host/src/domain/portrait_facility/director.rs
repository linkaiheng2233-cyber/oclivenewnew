...
use crate::domain::portrait_facility::rule::{
    resolve_visual_state_rule, validate_asset_id,
};
...
pub async fn pick_portrait_with_catalog(
    ...
    let mut visual_state_id = if portrait_director_enabled() {
        ...
        resolve_visual_state_rule(catalog, &fallback_base_from_emotion(bot_emotion, recent_turns))
            .unwrap_or_else(|| "neutral_default".to_string())
    };
    ...
    portrait_tag = apply_persona_event_overrides(
        portrait_tag,
        user_emotion_str,
        recent_events,
        personality,

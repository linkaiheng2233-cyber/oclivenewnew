...
use crate::models::dto::{TheaterSceneRequest, TheaterSceneResponse, TheaterScriptLine, TheaterTweak};
...
pub async fn generate_patch_scene(
    ...
    let Some(ctx) = resolve_patch_context(req) else {
        return Ok(patch_fallback_response(req, state.ollama_model.as_str(), "fallback", "patch_no_tweak"));
    };
    ...
    let (llm, model) = match resolve_theater_llm(state, req).await {
        ...
        Err(e) => {
            ...
            return Ok(patch_merge_fallback(&ctx, req, state.ollama_model.as_str(), "fallback"));
        }
    };
    ...
    let raw = match tokio::time::timeout(timeout, llm.generate(model.as_str(), prompt.as_str())).await {
        Ok(Ok(t)) => t,
        Ok(Err(e)) => {
    ...
    let retry_raw =
        match tokio::time::timeout(timeout, llm.generate(model.as_str(), strict_prompt.as_str())).await
        {
            Ok(Ok(t)) => t,
            Ok(Err(e)) => {
...
fn patch_merge_fallback(
    ...
    let beats = merge_patch_beats(&ctx.prefix_beats, ctx.canned_patch.clone(), &ctx.skeleton_tail);
...
fn try_merge_patch(
    ...
    let patch = parse_patch_prose(
        raw,
        req,
        lead,
        ctx.chip_slug.as_str(),
        max_lines,
    )?;
    if patch.is_empty() {
        return None;
...
pub(crate) fn build_patch_prompt(
    ...
    parts.push(drama_guardrails::drama_guardrails_full(req.theater_scene.as_deref()));

    if !persona_a.is_empty() || !persona_b.is_empty() {
        parts.push(String::new());
        parts.push("【人设摘要 · 必须贴合】".to_string());
        if !persona_a.is_empty() {
            parts.push(format!("{}（cast a）：{}", req.cast_a.name.trim(), persona_a));
        }
        if !persona_b.is_empty() {
            parts.push(format!("{}（cast b）：{}", req.cast_b.name.trim(), persona_b));
        }
    }
...
fn build_style_examples_block(
    ...
    for b in prefix.iter().rev().take(2).collect::<Vec<_>>().into_iter().rev() {
...
pub fn parse_patch_prose(
    ...
    for row in trimmed.split('\n').map(str::trim).filter(|s| !s.is_empty()) {
        ...
        let (name, text) = if let Some((n, t)) = row.split_once('：').or_else(|| row.split_once(':')) {
            (n.trim(), t.trim())
        } else {
            (fallback_name, row)
        };

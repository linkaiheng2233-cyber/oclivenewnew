//! 应用级设置（`app_settings`），供受控桥接等宿主更新。
//!
//! 错误为 **纯字符串**，与历史 `update_settings` / 插件桥契约一致（无前缀 `[CODE]`）。

use crate::models::interaction_mode::InteractionMode;
use crate::state::KernelAppState;
use serde_json::{json, Value};

pub async fn update_settings(state: &KernelAppState, params: &Value) -> Result<Value, String> {
    let obj = params
        .as_object()

...
pub fn validate_penetration_templates_config(value: &serde_json::Value) -> Vec<String> {
    ...
    errs.extend(validate_optional_string("diary_header", &cfg.diary_header, 500));
    errs.extend(validate_optional_string("diary_path", &cfg.diary_path, 260));
    errs.extend(validate_optional_string("letter_template", &cfg.letter_template, 2000));
    errs.extend(validate_optional_string("idle_message", &cfg.idle_message, 500));
    let _ = cfg.enabled;
    errs
}

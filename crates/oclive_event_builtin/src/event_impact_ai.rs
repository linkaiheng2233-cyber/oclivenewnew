...
use crate::affect_axis::softness_coldness_volatility;
use crate::event_detector::EventDetector;
use crate::json_loose::extract_json_object;
...
use oclive_kernel_models::{
    Event, EventImpactEstimate, EventType, KnowledgeEventAugment, PersonalityVector,
};
use serde_json::Value;
use std::sync::Arc;

#[inline]
fn personality_stability_index(personality: &PersonalityVector) -> f64 {
...
pub fn soften_impact_factor(ai_impact_factor: f64, personality: &PersonalityVector) -> f64 {
    ...
    let stability = personality_stability_index(personality);
...
mod tests {
    ...
    async fn estimate_event_impact_falls_back_when_parse_fails() {
        let llm: Arc<dyn LlmClient> = Arc::new(MockTagLlm {

...
- **三主打产品（2026-06）**：**Chat Pro**（`desktop` · Tauri · open ceiling）· **VS Code Flash**（`vscode` · 显式六槽 builtin · Pro 简洁版）· **AI Theater**（`theater` · Deferred，见 [`handoff/THEATER_DISTRO_ROADMAP.md`](handoff/THEATER_DISTRO_ROADMAP.md)）；`desktop-chat` = dev lab only（`examples/`）。

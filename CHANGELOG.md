...
- **ChatPro 应用图标体系**：桌面图标改为苹果式连续圆角底板，使用固定灰白棋盘格承载“左侧平滑插画 / 右侧像素赛博”角色形象；图形占用率提高到约 97%，并同步生成 PNG、ICO、ICNS 与 Windows 商店尺寸，圆角外保持真实透明。
- **示例角色表现收敛**：继续优化社区示例角色的人设表达、关系边界和安全护栏，使角色口吻更鲜明，同时不改变角色包 schema 或运行时兼容线。

### Fixed

- **快速轮次情绪与立绘状态**：快速连续对话中的短期情绪会按轮次衰减，气泡情绪与立绘情绪分开维护，避免角色长期卡在“开心”等单一立绘。

### Compatibility

- **无破坏性契约变化**：角色包格式、内核 API、数据库迁移和插件协议均未升版；现有 0.5.x 角色包继续兼容。

## [0.5.1] - 2026-08-22

### Added

- **安装自修复与结构化支持报告**：设置页新增保守型安装修复入口，同时随 MSI/NSIS 安装包发布独立 `Repair-AILiveChatPro.cmd` / `.ps1`。修复器会检查角色、目录插件、应用数据目录与 Ollama 连通性，按范围、类别、严重级别和稳定错误码汇总全部问题并保存 JSON 报告；只补齐缺失资源和目录，不删除用户角色、插件、聊天或设置。

### Fixed

- **桌面发行资源闭环**：正式构建会从受控源目录重新装配 7 个生产插件，并同时打包角色、支持脚本和内核资源；修复 NSIS 安装布局下角色目录定位与“插件目录未找到/未扫描”问题。生成的插件副本不再纳入版本控制，避免构建后出现陈旧资源和幽灵差异。
- **角色立绘显示与真实回归**：Tauri 图片 CSP 明确允许角色立绘使用的 `blob:` / `data:` / asset 来源，并新增角色详情挂载测试和发行资源 E2E 断言，防止“配置已保存但界面未真实显示”。
- **Windows 后台进程体验**：内核 sidecar、目录插件、MCP stdio、llama-server、Git 安装辅助和 NVIDIA 探测统一使用无窗口子进程标志，不再在日常使用中反复弹出终端窗口。

### Compatibility

- **无破坏性契约变化**：角色包 schema、内核 API 与数据库迁移版本保持不变；0.5.x 角色包与编写器继续兼容。

## [0.5.0] - 2026-08-20

### Added

- **A.I.Live Chat Pro 0.5.0 内测首发阵容**：桌面安装器、窗口与发行版展示名统一为 `A.I.Live Chat Pro`；整体内核与技术叙事继续使用“A.I.Live—角色动脉织机”。新增 `claude`、`gpt-dragon`、`nailong` 三个社区创作、非官方角色包并纳入 Git 与桌面发行资源；仓库名、内部协议和应用标识符保持兼容。

- **角色包回复模式独立通道 v1**：新增 `reply_mode` 侧通道，v1 支持 `single` 与 `burst`；角色包 `config.json` 可配置段数、分隔符（默认 `+++`）、段间展示延迟与流式策略。宿主在 `post_llm` 之后按独立行分隔符切分并剥离标记，一条助手消息落库并携带 `reply_segments` 元数据，前端按段渲染多个气泡并延迟展示，语音朗读拼接正文。AN-94 角色包开启双发模式并更新人设与质量锚点；模型漏分隔符时优雅回退为单段，撤回、删除与重新生成仍按一轮整体处理。

- **领域感知 CI Stage 3.1 开发期落地**：安全的草稿 PR 现按受信影响计划选择既有 job，并使用不能满足 main 分支保护的 `ci-draft-gate`；转为 ready 会在当前提交重新触发，除已有真实证据的纯文档 Canary 外仍跑全量正式 `ci-gate`，Push、高风险/未知路径、warning 与规划异常继续 fail-safe 全量。Git changed path 改为 NUL 分隔端到端传输，中文角色路径不再因 Git 引号/八进制转义误回退。每次全量 run 自动上传保留 90 天的同 workflow SHA Compare 证据，选择性 run 明确为非权威执行样本；影子语料扩为 20 场景（17 targeted / 3 fail-safe），覆盖 Unicode、混合域与 merge 策略。
...
- **领域感知 CI Stage 1 影子规划基础**：新增严格版本化的 `oclive.module.json`、中央影响图与受信验证目录，并由纯 Rust 规划器以“路径定位直接模块 + 语义影响闭包”生成稳定、可解释的 `plan.json`；未知路径、损坏描述、未知 required 扩展及中央高风险规则均 fail-safe 到当前策略全量。`oclive ci plan/explain` 支持 Git diff、显式路径和 Markdown 摘要；Stage 1 最初只上传 Job Summary/artifact 且不跳 job，现行执行边界已演进为上方 Stage 3.1。脚手架仍只负责生成/预检相同元数据。
...
- **R18 本机设置撤销、扩展隔离与双层硬上限**：Chat Pro 成人设置新增“重置成年确认与 R18 设置”，会先取消在途分拍并恢复本机开关、角色选择、节拍和队列默认值，同时保留已有对话、记忆及取消失败时的可重试 tombstone。损坏的 `adult_extension.json` 现在只禁用该扩展，基础角色仍可普通加载，设置页显示醒目诊断。用户节拍输入限制为 500–60000ms、队列限制为 1–8；旧值与模型建议安全收敛，内核同时拒绝同一生成中的第 9 个未消费分拍。成人基座与 LoRA 继续各自独立确认，并补齐两条后端拒绝回归。
...
- **CI 执行通道分流**：将 `loom`、`fuzz`、`cli-bench`、`visual-presentation-smoke` 与原生 `e2e-tauri` 从 PR/Push 主工作流迁至独立 `nightly-advisory.yml`，支持每日定时与按验证器手动触发。Nightly 失败在自身工作流真实变红但不阻塞 main，并为 CLI 性能与 fuzz/E2E 失败保留 artifact；主 CI 保留硬门禁，选择性执行由上方 Stage 3.1 的受信策略控制。
- **影子规划样本契约**：严格版本化的模拟语料现为 20 场景，并由 `ci:shadow-samples` 证据收集器覆盖文档、shared、角色包、目录插件、内核契约、脚手架、examples、Nightly 分层、CI 控制面、依赖锁、未知路径、Unicode/NUL 输入、混合域和 merge 策略。仓库契约测试锁定模块闭包、验证器、工作流坐标及 fail-safe 原因；生成证据明确标记“仅模拟、未执行验证器、不能证明零漏选”。
...
- **CI 定向重跑降噪**：`.github/workflows/ci-rerun-flake.yml` 不再由每个 CI 完成事件自动创建工作流记录；维护者须手动指定失败 run ID，且仅当失败项为 `rust (<matrix-os>)` 加聚合 `ci-gate`、原 run 未重试过时才定向重跑一次。其他失败保留现场，不用重跑掩盖确定性问题。

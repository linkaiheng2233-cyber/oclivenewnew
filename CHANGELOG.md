...
- **统一资源协调器与适配器注册表**：桌面 Host 现在以单一协调器汇总 NVIDIA 显存快照、优先级、原子预留与租约诊断，并通过 HostProfile 控制安全余量和无法探测时的策略；托管 `llama-server`、外部 Ollama 活动、performance 活动观察器和官方 CosyVoice2 已接入宿主 Resource Adapter Registry，资源诊断 v2 会区分控制权、adapter-local 档位、驻留能力、生命周期动作和当前 `profile_id`。蓝图与角色包只声明能力和理想配置，不直接支配 GPU；观测型 LLM 热路径不会额外调用 `nvidia-smi`，尚未实现的自动切档也不会被注册表冒充为可用。

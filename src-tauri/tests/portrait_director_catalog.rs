...
fn write_catalog_role(base: &PathBuf, role_id: &str) {
...
async fn narrative_hint_changes_visual_state_id() {
    ...
    write_catalog_role(&dir.path().to_path_buf(), "director_role");
...
async fn without_hint_prefers_happy_id() {
    ...
    let dir = tempfile::tempdir().unwrap();
    write_catalog_role(&dir.path().to_path_buf(), "director_role2");
    let storage = RoleStorage::new(dir.path().to_path_buf());

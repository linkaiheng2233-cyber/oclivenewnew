import type { ChatMessage } from '@oclive/shared/stores/chatStore'
...
export function messageMapToImportBuckets(map: RoleSceneMessageMap): import('@oclive/shared/api/chatStorage').ImportChatBucket[] {
  const buckets: import('@oclive/shared/api/chatStorage').ImportChatBucket[] = []
  for (const [roleId, roleBucket] of Object.entries(map)) {
    if (Array.isArray(roleBucket)) {
      buckets.push({
        role_id: roleId,

...
const hintText = computed(() =>
  props.waitingPhase === 'model'
    ? t('theater.think.waitingHint')
    : t('theater.think.hint'),
)
</script>
  ...
  <Transition name="think-fade">
    ...
    >
      <div class="think-chain__head">
        ...
        <span class="think-chain__dot" aria-hidden="true" />
        <span class="think-chain__dot" aria-hidden="true" />
        <span class="think-chain__title">{{ title }}</span>
      </div>
...
.think-chain {
  ...
  margin: 0 auto;
  width: min(var(--theater-stage-max-w, 720px), 100%);
  padding: var(--tool-space-3, 12px) var(--tool-space-4, 16px);
  border-top: 1px solid var(--tool-divider, var(--border-light));
  border-radius: var(--tool-radius-lg, 12px) var(--tool-radius-lg, 12px) 0 0;

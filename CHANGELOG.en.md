...
- **Scaffold Package v1 and a focused CLI surface**: added the independent `oclive_scaffold` contract crate, strict JSON Schemas, project/user/official local discovery, configurable priority, version rejection, source plus SHA-256 locking, path-escape checks, and atomic `.oclive/scaffold.lock.json`; four compiled official fallback packages only describe existing generators. `oclive scaffold list/inspect/validate/resolve` is diagnostic-only: Stage 2A does not use the network, execute third-party entries, or resolve composition, and it rejects `ci.*` capabilities plus third-party `com.oclive.*` namespaces. Default help now exposes 15 stable entries while ten experimental commands and legacy project-archive `template` remain callable but hidden; domain-aware CI registers a separate `oclive.scaffold` module whose impact only propagates to `oclive.cli`.

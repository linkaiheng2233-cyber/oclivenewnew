...
- **Chat history vanished in story scenes**: `hydrateFromStorage` resolves the scene with the default `pure_chat` mode (always `home`) before role info loads, while `uiStore.sceneId` still holds the persisted immersive scene (e.g. `company`); the later `applySceneChange` then short-circuited on `prev === next` and never loaded the target bucket, blanking the chat. Now reloads whenever the target bucket is unloaded (data was never lost); guard `chatStoreScene.test.ts`, see [`CHAT_STORAGE_ARCHITECTURE.md`](handoff/CHAT_STORAGE_ARCHITECTURE.md).

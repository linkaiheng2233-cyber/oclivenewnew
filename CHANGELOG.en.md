...
- **Unified Resource Coordinator, stage 1**: the desktop Host now owns one coordinator for NVIDIA VRAM snapshots, priorities, atomic reservations, lease lifecycle, and diagnostics, with HostProfile policy for safety headroom and unavailable telemetry. Managed `llama-server`, observed external Ollama activity, and the bundled CosyVoice2 path use adapters with explicit control boundaries. Blueprints and role packs describe capabilities and desired configuration without controlling the GPU directly; read-only resource diagnostics and execution-plan states are exposed, while observed LLM hot paths do not add `nvidia-smi` probes.

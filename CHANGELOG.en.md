...
- **Domain-aware CI Stage 3.1 development rollout**: safe draft PRs now select existing jobs from the trusted impact plan and publish `ci-draft-gate`, which cannot satisfy main's branch protection. Marking the PR ready retriggers the current commit; only the already-evidenced docs-only canary remains selective, while every other ready PR runs the full formal `ci-gate`. Pushes, high-risk or unknown paths, warnings, and planner failures remain fail-safe full. Git changed paths now travel end to end as NUL-delimited UTF-8, preventing Chinese role paths from falling back because of Git quoting or octal escapes. Every full run automatically uploads 90-day same-workflow-SHA Compare evidence, while selective runs are explicitly non-authoritative execution samples. The shadow corpus now has 20 scenarios (17 targeted / 3 fail-safe) spanning Unicode, mixed domains, and merge policies.
...
- **Domain-aware CI Stage 1 shadow-planning foundation**: added strict versioned `oclive.module.json` descriptors, a central impact map, and a trusted validation catalog. A pure Rust planner combines path-based direct ownership with semantic impact closure and emits stable, explainable `plan.json`; unknown paths, broken descriptors, unsupported required extensions, and central high-risk rules fail safe to the active policy's full set. `oclive ci plan/explain` accepts Git diffs or explicit paths and renders Markdown. Stage 1 initially only uploaded a Job Summary/artifact and skipped no jobs; the active execution boundary has since advanced to Stage 3.1 above. The scaffold still only generates and preflights the same metadata.
...
- **Revocable local R18 settings, extension isolation, and two-layer hard bounds**: Chat Pro Adult Settings now provides “Reset adult confirmation and R18 settings,” which first cancels staged generation and then restores local gates, role choices, pacing, and queue defaults while preserving conversations, memories, and retryable tombstones from failed cancellation. A broken `adult_extension.json` now disables only that extension; the base role remains loadable in ordinary mode and Settings shows a prominent diagnostic. User pacing input is limited to 500–60000 ms and queue capacity to 1–8; legacy values and model suggestions are safely clamped, while the kernel rejects a ninth unconsumed beat in the same generation. Adult base-model and LoRA acknowledgements remain independent and now have direct backend rejection regressions.
...
- **Quieter targeted CI reruns**: `.github/workflows/ci-rerun-flake.yml` no longer creates a workflow entry after every completed CI run. A maintainer must manually supply a failed run ID, and one targeted rerun is allowed only when the failure set is `rust (<matrix-os>)` plus the aggregate `ci-gate` and the original run has not already been retried. Every other failure preserves the original evidence rather than being masked by a retry.

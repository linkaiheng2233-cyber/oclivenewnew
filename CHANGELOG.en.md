...
- **Unified Resource Coordinator and adapter registry**: the desktop Host now owns one coordinator for NVIDIA VRAM snapshots, priorities, atomic reservations, lease lifecycle, and diagnostics, with HostProfile policy for safety headroom and unavailable telemetry. Managed `llama-server`, observed external Ollama activity, the performance activity observer, and bundled CosyVoice2 register through the host Resource Adapter Registry; resource diagnostics v2 distinguish control ownership, adapter-local profiles, residency support, truthful lifecycle actions, and the current `profile_id`. Blueprints and role packs describe capabilities and desired configuration without controlling the GPU directly. Observed LLM hot paths add no `nvidia-smi` probes, and the registry does not claim that unimplemented automatic profile switching is available.
